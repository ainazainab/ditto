...
try {
    ...
    if ($_.Exception.Response) {
        ...
        if ($code -eq 401) {
            ...
            Write-Host "Result: Request rejected - access forbidden" -ForegroundColor Green
            Write-Host "Status Code: 403 Forbidden" -ForegroundColor Green
            Write-Host "[OK] SECURE: System properly enforces authentication" -ForegroundColor Green
        } else {
            Write-Host "Result: Request rejected" -ForegroundColor Green
            Write-Host "Status Code: $code" -ForegroundColor Green
            Write-Host "[OK] SECURE: System properly enforces authentication" -ForegroundColor Green
        }
    } else {
        $errorMsg = $_.Exception.Message
        Write-Host "Result: Connection error" -ForegroundColor Yellow
        Write-Host "Error: $errorMsg" -ForegroundColor Gray
        Write-Host "[?] UNKNOWN: Connection failed" -ForegroundColor Yellow
    }
}
