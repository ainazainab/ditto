...
if ($containerName) {
    ...
    Write-Host ""
    if ($vulnerable) {
        Write-Host "Result: Network isolation failure detected" -ForegroundColor Red
        foreach ($issue in $issues) {
            Write-Host "  - $issue" -ForegroundColor Red
        }
        Write-Host "[X] VULNERABILITY: Containers can access each other directly - network isolation compromised" -ForegroundColor Red
    } else {
        Write-Host "Result: Network isolation is properly configured" -ForegroundColor Green
        Write-Host "[OK] SECURE: Containers are properly isolated" -ForegroundColor Green
    }
} else {
